...
st.title("📈 Advanced Stock Price Visualizer & Predictor")

# Sidebar inputs
st.sidebar.header("⚙️ Settings")
tickers = st.sidebar.text_input("Enter stock tickers (comma separated)", "AAPL, GOOGL, MSFT")
...
future_days = st.sidebar.slider("Days to Predict (LSTM)", 1, 30, 7)
...
for i, t in enumerate(tickers):
    ...
    if df is not None and not df.empty:
        ...
        cols[i].metric(label=t, value="N/A", delta="N/A")
...
for t in tickers:
    ...
    if df is not None and not df.empty:
        ...
        if show_ma:
            fig.add_trace(go.Scatter(x=df.index, y=df[f"SMA_{sma_period}"], name=f"SMA {sma_period}"))
            fig.add_trace(go.Scatter(x=df.index, y=df[f"EMA_{ema_period}"], name=f"EMA {ema_period}"))
        fig.update_layout(title=f"{t} Price Chart", xaxis_title="Date", yaxis_title="Price (USD)")
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning(f"⚠️ No data found for {t}")
...
predict_ticker = st.text_input("Enter ticker for prediction", "AAPL").upper()

if st.button("Predict Future Prices"):
    try:
        df_pred = yf.download(predict_ticker, start="2015-01-01", end=datetime.date.today())

        # Auto-fallback if no valid data
        if df_pred.empty or 'Close' not in df_pred.columns:
            st.warning(f"No valid data found for {predict_ticker}. Defaulting to AAPL.")
            predict_ticker = "AAPL"
            df_pred = yf.download("AAPL", start="2015-01-01", end=datetime.date.today())

        data = df_pred.filter(['Close']).dropna()

        if data.empty:
            st.warning(f"No valid 'Close' price data available for {predict_ticker}.")
        else:
            dataset = data.values
            ...
            if len(x_train) == 0:
                st.warning("Insufficient data to train LSTM model.")
            else:
